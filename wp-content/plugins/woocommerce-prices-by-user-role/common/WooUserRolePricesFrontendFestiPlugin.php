...
class WooUserRolePricesFrontendFestiPlugin extends WooUserRolePricesFestiPlugin
{
    ...
    public function onDisplayCustomerTotalSavingsAction($total)
    {
        ...
        $totalSavings = $this->fetchTotalSavings($retailTotal, $userTotal);

        $vars = array(
            'regularPrice' => $this->fetchPrice($retailTotal),
            'userPrice' => $this->fetchPrice($userTotal),
            'userDiscount' => $totalSavings
        );
    ...
    public function fetchTotalSavings($retailTotal, $userTotal)
    {
        if(!$retailTotal || !$userTotal) {
            return 0;
        }
    ...
    )
    {
        ...
        if (!$this->_hasAvailableRoleToViewPrices($this->settings)
            || !$hasAvaliableRole) {
            return '';
        }
        ...
        $vars = array(
            'regularPrice' => $this->fetchRegularPrice($product->variation_id),
    ...
    public function onDisplayPriceContentForSingleProductFilter($content, $item)
    {
        ...
        $vars = array(
            'regularPrice' => $this->fetchRegularPrice($item->id),
    ...
    public function fetchRegularPrice($productId)
    {
        $price = $this->getRegularPrice($productId);

        return $this->fetchPrice($price);
    } // end fetchRegularPrice
    ...
    public function fetchUserPrice($product)
    {
        ...
        return $this->fetchPrice($userPrice);
    } // end fetchUserPrice
    ...
    {
        return array_key_exists('userIdForAjax', $_SESSION);
