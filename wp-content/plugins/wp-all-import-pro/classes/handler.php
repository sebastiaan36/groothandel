...
class PMXI_Handler extends PMXI_Session {
    ...
    public function clean_session( $import_id = 'new' ){
		...
		$wpallimport_session_expires = $wpdb->get_results( "SELECT option_name, option_value FROM $wpdb->options WHERE option_name LIKE '_wpallimport_session_expires_". esc_sql($import_id) ."_%'" );			
